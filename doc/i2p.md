...
```
dogecashd -i2psam=127.0.0.1:7656
```
...
`dogecash-cli help logging` for more information.
...
`dogecash-cli -addrinfo` to see the number of I2P addresses known to your node.
...
- in the output of `dogecash-cli -netinfo` peer connections dashboard

To see which I2P peers your node is connected to, use `dogecash-cli -netinfo 4`
or the `getpeerinfo` RPC (e.g. `dogecash-cli getpeerinfo`).
