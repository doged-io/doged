...
Below are some notes on how to build doged for Windows.

The options known to work for building doged on Windows are:

* On Linux, using the [Mingw-w64](https://mingw-w64.org/doku.php) cross compiler tool chain. Debian Bullseye is recommended
and is the platform used to build the doged Windows release binaries.
...
The cmake version packaged with Ubuntu Bionic is too old for building Building doged.
...
Note that for WSL the doged source path MUST be somewhere in the default mount file system, for
...
It's not possible to build the doged code using the win32 version of the Mingw-w64 cross compilers (at least not without
modifying headers in the doged source code).
