...
The [/test/](/test/) directory contains integration tests that test dogecashd
...
dogecashd and bitcoin-qt by interacting with them through the RPC and P2P
interfaces.
...
with the dogecashd(s) being tested in a callback-based event loop; the other
implements the test logic.

- `P2PConnection` is the class used to connect to a dogecashd.  `P2PInterface`
...
Test objects for interacting with a dogecashd node over the p2p interface.
