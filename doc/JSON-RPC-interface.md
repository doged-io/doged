...
The headless daemon `dogecashd` has the JSON-RPC API enabled by default, the GUI
`dogecash-qt` has it disabled by default. This can be changed with the `-server`
...
dogecash-cli createwallet mywallet false false "" false false true

# "params": {"wallet_name": "mywallet", "load_on_startup": true}
dogecash-cli -named createwallet wallet_name=mywallet load_on_startup=true

# "params": {"args": ["mywallet"], "load_on_startup": true}
dogecash-cli -named createwallet mywallet load_on_startup=true
```

## Versioning

The RPC interface might change from one major version of Bitcoin ABC to the
...
- **Securing remote network access:** You may optionally allow other
  ...
  `dogecashd -help` for more information about these settings and other
