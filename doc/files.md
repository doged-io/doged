...
`./`               | `bitcoin.conf`        | User-defined [configuration settings](bitcoin-conf.md) for `dogecoind` or `dogecoin-qt`. File is not written to by the software and must be created manually. Path can be specified by `-conf` option
