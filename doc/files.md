...
The data directory is the default location where the doged files are stored.
...
---------|--------------------
Linux    | `$HOME/.doge/`
macOS    | `$HOME/Library/Application Support/doged/`
Windows  | `%APPDATA%\doged\` <sup>[\[1\]](#note1)</sup>
...
`./`               | `banlist.dat`         | Stores the addresses/subnets of banned nodes (deprecated). `doged` or `doge-qt` no longer save the banlist to this file, but read it on startup if `banlist.json` is not present.
`./`               | `banlist.json`        | Stores the addresses/subnets of banned nodes.
`./`               | `dogecoin.conf`        | User-defined [configuration settings](bitcoin-conf.md) for `doged` or `doge-qt`. File is not written to by the software and must be created manually. Path can be specified by `-conf` option
`./`               | `doged.pid`        | Stores the process ID (PID) of `doged` or `doge-qt` while running; created at start and deleted on shutdown; can be specified by `-pid` option
`./`               | `debug.log`           | Contains debug information and general logging generated by `doged` or `doge-qt`; can be specified by `-debuglogfile` option
...
`doge-qt` uses [`QSettings`](https://doc.qt.io/qt-5/qsettings.html) class; this implies platform-specific [locations where application settings are stored](https://doc.qt.io/qt-5/qsettings.html#locations-where-application-settings-are-stored).

## Legacy subdirectories and files

These subdirectories and files are no longer used by doged:
