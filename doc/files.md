...
`./`               | `bitcoin.conf`        | User-defined [configuration settings](bitcoin-conf.md) for `dogecashd` or `bitcoin-qt`. File is not written to by the software and must be created manually. Path can be specified by `-conf` option
`./`               | `dogecashd.pid`        | Stores the process ID (PID) of `dogecashd` or `bitcoin-qt` while running; created at start and deleted on shutdown; can be specified by `-pid` option
`./`               | `debug.log`           | Contains debug information and general logging generated by `dogecashd` or `bitcoin-qt`; can be specified by `-debuglogfile` option
