...
class BIP65Test(BitcoinTestFramework):
    ...
    def run_test(self):
        ...
        block = create_block(
            ...
            block_time,
            version=VERSION_CHAIN_ID_BITS | 3,
        ...
        tip = block.hash_int
        block_time += 1
        block = create_block(tip, create_coinbase(CLTV_HEIGHT), block_time, version=VERSION_CHAIN_ID_BITS | 3)
        block.solve()

        with self.nodes[0].assert_debug_log(
            expected_msgs=[f"{block.hash_hex}, bad-version(0x00620003)"]
        ):
            peer.send_and_ping(msg_block(block))
