...
class AbcRTTActivationTest(BitcoinTestFramework):
    ...
    def run_test(self):
        ...
        def submit_new_block_then_accept(expect_initially_accepted):
            ...
            block = create_block(
                int(tip, 16), cb, node.getblock(tip)["time"] + 1, version=VERSION_CHAIN_ID_BITS | 4
            )
            block.solve()
            ...
            assert_response([AvalancheVote(expected_vote, block.hash_int)])
            ...
            poll_node.send_poll([block.hash_int])
            assert_response([AvalancheVote(AvalancheVoteError.ACCEPTED, block.hash_int)])
