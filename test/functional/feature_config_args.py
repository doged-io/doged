...
class ConfArgsTest(BitcoinTestFramework):
    ...
    def test_ignored_conf(self):
        self.log.info(
            "Test error is triggered when the datadir in use contains a dogecoin.conf file that would be ignored "
        ...
        node.assert_start_raises_init_error(
            ...
            re.escape(
                f'Error: Data directory "{node.datadir}" contains a "dogecoin.conf" file which is ignored, because a '
        ...
        # the same dogecoin.conf that would be loaded anyway does not trigger an
        # error.
        self.start_node(0, [f"-conf={node.datadir}/dogecoin.conf"])
        self.stop_node(0)
    ...
    def test_ignored_default_conf(self):
        ...
        self.log.info(
            "Test error is triggered when dogecoin.conf in the default data directory sets another datadir "
            "and it contains a different dogecoin.conf file that would be ignored"
        )
        ...
        # Write a dogecoin.conf file in the default data directory containing a
        # datadir= line pointing at the node datadir. This will trigger a
        # startup error because the node datadir contains a different
        # dogecoin.conf that would be ignored.
        node = self.nodes[0]
        (default_datadir / "dogecoin.conf").write_text(f"datadir={node.datadir}\n")
        ...
        node.assert_start_raises_init_error(
            ...
            re.escape(
                f'Error: Data directory "{node.datadir}" contains a "dogecoin.conf" file which is ignored, because a '
                f'different configuration file "{default_datadir}/dogecoin.conf" from data directory "{default_datadir}" '
                f"is being used instead."
            )
            + r"[\s\S]*",
            env=env,
