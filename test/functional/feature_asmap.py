...
5. `dogecashd -asmap` restart with an addrman containing new and tried entries

6. `dogecashd -asmap` with no file specified and a missing default asmap file

7. `dogecashd -asmap` with an empty (unparsable) default asmap file
...
class AsmapTest(BitcoinTestFramework):
    ...
    def test_default_asmap(self):
        ...
        for arg in ["-asmap", "-asmap="]:
            self.log.info(f"Test dogecashd {arg} (using default map file)")
            self.stop_node(0)
    ...
    def test_asmap_interaction_with_addrman_containing_entries(self):
        self.log.info(
            "Test dogecashd -asmap restart with addrman containing new and tried entries"
        )
        self.stop_node(0)
        shutil.copyfile(self.asmap_raw, self.default_asmap)
