...
"""Test dogecashd with different proxy configuration.

Test plan:
- Start dogecashd's with different proxy configurations
- Use addnode to initiate connections
- Verify that proxies are connected to, and the right connection command is given
- Proxy configurations to test on dogecashd side:
...
class ProxyTest(BitcoinTestFramework):
    ...
    def node_test(self, node, proxies, auth, test_onion=True):
        ...
        # Note: dogecashd's SOCKS5 implementation only sends atyp DOMAINNAME,
        ...
        if self.have_ipv6:
            ...
            assert isinstance(cmd, Socks5Command)
            # Note: dogecashd's SOCKS5 implementation only sends atyp
