...
class RejectLowDifficultyHeadersTest(BitcoinTestFramework):
    ...
    def test_chains_sync_when_long_enough(self):
        ...
        self.log.info(
            "Generate more blocks to satisfy node1's minchainwork requirement, and "
            "verify node2 still has no new headers in headers tree"
        )
        with self.nodes[2].assert_debug_log(
            expected_msgs=["Ignoring low-work chain (height=15)"]
        ):
            self.generate(
                self.nodes[0],
