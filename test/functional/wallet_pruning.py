...
class WalletPruningTest(BitcoinTestFramework):
    def set_test_params(self):
        ...
        self.extra_args = [
            ...
            [],
            # node dedicated to testing pruning
            ["-prune=550"],
        ]
    ...
    def mine_large_blocks(self, node, n):
        ...
        for _ in range(n):
            ...
            for n in self.nodes:
                if n.running:
                    # Update node's time to accept future blocks
                    n.setmocktime(self.nTime)
        self.sync_all()
    ...
    def get_birthheight(self, wallet_file):
        ...
        with open(
            os.path.join(self.nodes[0].datadir, wallet_file), "r", encoding="utf8"
        ) as f:
            for line in f:
    ...
    def has_block(self, block_index):
        ...
        return os.path.isfile(
            os.path.join(
                self.nodes[1].datadir, self.chain, "blocks", f"blk{block_index:05}.dat"
            )
        )
