...
from test_framework.cdefs import LEGACY_MAX_BLOCK_SIZE, MAX_BLOCK_SIGOPS
...
def get_legacy_sigopcount_tx(tx, fAccurate=True):
    count = 0
    for i in tx.vout:
        count += count_tx_sigops(i.scriptPubKey, fAccurate)
    for j in tx.vin:
        # scriptSig might be of type bytes, so convert to CScript for the
        # moment
        count += count_tx_sigops(j.scriptSig, fAccurate)
    return count
...
class FullBlockSigOpsTest(BitcoinTestFramework):
    ...
    def run_test(self):
        ...
        self.coinbase_key.set(b"correct horse battery staple 936", True)
        ...
        lots_of_multisigs = CScript(
            [OP_CHECKMULTISIG] * ((MAX_BLOCK_SIGOPS - 1) // 20) + [OP_CHECKSIG] * 19
        )
        ...
        redeem_script = CScript(
            [self.coinbase_pubkey] + [OP_2DUP, OP_CHECKSIGVERIFY] * 5 + [OP_CHECKSIG]
        )
        ...
        for i in range(1, numTxs + 1):
            ...
            sighash = SignatureHashForkId(
                redeem_script, tx, 1, SIGHASH_ALL | SIGHASH_FORKID, lastAmount
            )
        ...
        b40_sigops_to_fill = (
            MAX_BLOCK_SIGOPS - (numTxs * b39_sigops_per_output + sigops) + 1
        )
        tx = CTransaction()
        tx.vin.append(CTxIn(lastOutpoint, b""))
        ...
        # b75 succeeds because we put MAX_BLOCK_SIGOPS before the
        # element
        self.log.info(
