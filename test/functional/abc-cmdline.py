...
class ABC_CmdLine_Test(BitcoinTestFramework):
    ...
    def excessiveblocksize_test(self):
        ...
        self.check_subversion(r"/doged:.*\(EB2\.0; .*\)/")
        ...
        self.nodes[0].assert_start_raises_init_error(
            [
                f"-blockmaxsize={LEGACY_MAX_BLOCK_SIZE - 1}",
                f"-excessiveblocksize={LEGACY_MAX_BLOCK_SIZE - 1}",
            ],
            "Error: Excessive block size must be > 1,000,000 bytes (1MB)",
        )
        self.nodes[0].assert_start_raises_init_error(
            [f"-blockmaxsize={LEGACY_MAX_BLOCK_SIZE - 1}", "-excessiveblocksize=0"],
            "Error: Excessive block size must be > 1,000,000 bytes (1MB)",
        )
        self.nodes[0].assert_start_raises_init_error(
            [f"-blockmaxsize={LEGACY_MAX_BLOCK_SIZE - 1}", "-excessiveblocksize=-1"],
            "Error: Excessive block size must be > 1,000,000 bytes (1MB)",
        )
