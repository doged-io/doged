...
class NULLDUMMYTest(BitcoinTestFramework):
    def set_test_params(self):
        ...
        self.extra_args = [["-whitelist=127.0.0.1", "-legacyscriptrules=1"]]
    ...
    def run_test(self):
        ...
        # Block 2
        self.coinbase_blocks = self.generate(self.nodes[0], 2)
        coinbase_txid = []
        for i in self.coinbase_blocks:
            coinbase_txid.append(self.nodes[0].getblock(i)["tx"][0])
        # Block 429
        self.generate(self.nodes[0], 427)
        ...
        test1txs = [
            self.create_transaction(
                self.nodes[0], coinbase_txid[0], self.ms_address, 1000, 49990000
            )
        ]
        txid1 = self.nodes[0].sendrawtransaction(ToHex(test1txs[0]))
        test1txs.append(
            self.create_transaction(self.nodes[0], txid1, self.ms_address, 500)
        )
        txid2 = self.nodes[0].sendrawtransaction(ToHex(test1txs[1]))
        ...
        assert_raises_rpc_error(
            -26, NULLDUMMY_ERROR, self.nodes[0].sendrawtransaction, ToHex(test2tx)
        )
    ...
    def create_transaction(self, node, txid, to_address, amount, burn_amount=None):
        inputs = [{"txid": txid, "vout": 0}]
        outputs = {to_address: amount}
        if burn_amount is not None:
            outputs[ADDRESS_ECREG_UNSPENDABLE] = burn_amount
    ...
    def block_submit(self, node, txs, accept=False):
        block = create_block(
            self.tip, create_coinbase(self.lastblockheight + 1), self.lastblocktime + 1
        )
        block.nVersion = 4
        for tx in txs:
            tx.rehash()
