...
Based on abc-p2p-fullblocktest-sigops.py
"""
...
from test_framework.cdefs import MAX_BLOCK_SIGOPS, MAX_TX_SIGOPS, ONE_MEGABYTE
...
class FullBlockTest(BitcoinTestFramework):
    ...
    def create_tx(self, spend, value, script=CScript([OP_TRUE])):
        tx = create_tx_with_script(
            spend.tx, spend.n, b"", amount=value, script_pub_key=script
        )
        return tx
    ...
    def next_block(
        ...
        if spend is None:
            ...
            while current_block_size < block_size:
                ...
                tx_sigops = min(extra_sigops, script_length, MAX_TX_SIGOPS)
    ...
    def run_test(self):
        ...
        # Accept many sigops
        lots_of_checksigs = CScript([OP_CHECKSIG] * MAX_BLOCK_SIGOPS)
        block(19, spend=out[0], script=lots_of_checksigs, block_size=ONE_MEGABYTE)
        peer.send_blocks_and_test([self.tip], node)
        ...
        # Reject reject oversized block (40k sigops per block > 1MB and <= 2MB)
        block(
            21,
            spend=out[1],
            script=lots_of_checksigs,
            extra_sigops=MAX_BLOCK_SIGOPS,
        ...
        block(
            ...
            spend=out[3],
            script=lots_of_checksigs,
            extra_sigops=MAX_BLOCK_SIGOPS + 1,
            block_size=ONE_MEGABYTE + 1,
        )
        peer.send_blocks_and_test(
            [self.tip], node, success=False, reject_reason="bad-blk-length"
        ...
        too_many_tx_checksigs = CScript([OP_CHECKSIG] * (MAX_TX_SIGOPS + 1))
        block(29, spend=out[6], script=too_many_tx_checksigs, block_size=ONE_MEGABYTE)
        peer.send_blocks_and_test(
        ...
        private_key = ECKey()
        private_key.set(b"correct horse battery staple 936", True)
        ...
        # Sigops p2sh limit
        p2sh_sigops_limit = MAX_BLOCK_SIGOPS - count_tx_sigops(redeem_script, True)
        # Too many sigops in one p2sh txn
        too_many_p2sh_sigops = CScript([OP_CHECKSIG] * (p2sh_sigops_limit + 1))
        block(31, spend=out[7], block_size=100000)
        ...
        block(32, spend=out[8], block_size=100000)
        ...
        b33 = block(33, spend=out[9], block_size=100000)
        # 20001 sigops
        b33.vtx[0].vout.append(
            CTxOut(0, CScript([OP_CHECKMULTISIG] * 1000 + [OP_CHECKSIG]))
        )
        update_block(33, [])
        peer.send_blocks_and_test(
        ...
        update_block(33, [])
        peer.send_blocks_and_test([b33], node)
