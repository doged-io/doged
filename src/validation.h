...
struct PackageMempoolAcceptResult {
    ...
    /**
     * Package feerate, defined as the aggregated modified fees divided by the
     * total virtual size of all transactions in the package. May be unavailable
     * if some inputs were not available or a transaction failure caused
    ...
    explicit PackageMempoolAcceptResult(
        PackageValidationState state, CFeeRate feerate,
        std::map<const TxId, const MempoolAcceptResult> &&results)
        : m_state{state},
          m_tx_results(std::move(results)), m_package_feerate{feerate} {}
