...
BOOST_AUTO_TEST_CASE(select_peer_random) {
    for (int c = 0; c < 1000; c++) {
        ...
        for (size_t i = 0; i < size; i++) {
            ...
            const Score score = m_rng.randbits(3);
            max += score;
...
BOOST_AUTO_TEST_CASE(compact_slots_non_uniform_scores) {
    ...
    const std::array<Score, 4> scores{{1000000, 2000000, 3000000, 4000000}};
    std::array<PeerId, 4> peerids;
    for (int i = 0; i < 4; i++) {
