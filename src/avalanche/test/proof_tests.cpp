...
BOOST_AUTO_TEST_CASE(proof_random) {
    ...
    const Score maxScore = Proof::amountToScore(MAX_MONEY);

    for (int i = 0; i < 1000; i++) {
        // The randrage upper bound is exclusive, so we add 1 to include the
        // maximum score.
        const Score score = m_rng.randrange(maxScore + 1);
    ...
    const std::vector<TestVector> testCases{
        ...
        {"Duplicated UTXO",
         ...
         ProofId::fromHex("5d4919b43a1afb6acdeddaf1678397eaa10562125db6b911ec4e"
                          "35fd8598ad73"),
         // TODO: Need a proof that tests sum above MAX_MONEY has score
         // saturated.
