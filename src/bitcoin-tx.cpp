...
static int AppInitRawTx(int argc, char *argv[]) {
    ...
    if (argc < 2 || HelpRequested(gArgs) || gArgs.IsArgSet("-version")) {
        ...
        std::string strUsage = PACKAGE_NAME " dogecoin-tx utility version " +
                               FormatFullVersion() + "\n";
        ...
        if (gArgs.IsArgSet("-version")) {
            ...
            strUsage +=
                "\n"
                "Usage:  dogecoin-tx [options] <hex-tx> [commands]  Update "
                "hex-encoded bitcoin transaction\n"
                "or:     dogecoin-tx [options] -create [commands]   Create "
                "hex-encoded bitcoin transaction\n"
                "\n";
