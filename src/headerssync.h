...
public:
    ...
    enum class State {
        ...
        REDOWNLOAD,
        /**
         * We're done syncing with this peer and can discard any remaining
         * state
         */
        FINAL
    };
    ...
    /**
     * Return the block timestamp of the last header received during the
     * PRESYNC phase.
     */
    uint32_t GetPresyncTime() const { return m_last_header_received.nTime; }

    /**
     * Return the amount of work in the chain received during the PRESYNC
     * phase.
     */
     ...
     *
     * received_headers: headers that were received over the network for
     *                   processing.
     ...
     * ProcessingResult.success: set to false if an error is detected and the
     *                       sync is aborted; true otherwise.
     * ProcessingResult.request_more: if true, the caller is suggested to call
     *                       NextHeadersRequestLocator and send a getheaders
     *                       message using it.
     */
    ProcessingResult
    ProcessNextHeaders(const std::vector<CBlockHeader> &received_headers,
                       bool full_headers_message);

    /**
     * Issue the next GETHEADERS message to our peer.
     *
     * This will return a locator appropriate for the current sync object, to
     * continue the synchronization phase it is in.
     */
...
private:
    ...
    /**
     * Store the last block in our block index that the peer's chain builds
     * from
     */
    ...
    /**
     * A queue of commitment bits, created during the 1st phase, and verified
     * during the 2nd.
     */
     ...
     * chain could be, given the MTP rule.
     *
     * Any peer giving us more headers than this will have its sync aborted.
     * This serves as a memory bound on m_header_commitments.
     */
    uint64_t m_max_commitments{0};

    /**
     * Store the latest header received while in PRESYNC (initialized to
     * m_chain_start)
     */
