...
static bool TestSplitHost(const std::string &test, const std::string &host,
                          uint16_t port, bool validPort = true) {
    std::string hostOut;
    uint16_t portOut{0};
...
BOOST_AUTO_TEST_CASE(netbase_splithost) {
    ...
    BOOST_CHECK(TestSplitHost("127.0.0.1:22556", "127.0.0.1", 22556));
    BOOST_CHECK(TestSplitHost("[127.0.0.1]", "127.0.0.1", 0));
    BOOST_CHECK(TestSplitHost("[127.0.0.1]:22556", "127.0.0.1", 22556));
    BOOST_CHECK(TestSplitHost("::ffff:127.0.0.1", "::ffff:127.0.0.1", 0));
    BOOST_CHECK(
        TestSplitHost("[::ffff:127.0.0.1]:22556", "::ffff:127.0.0.1", 22556));
    BOOST_CHECK(TestSplitHost("[::]:22556", "::", 22556));
    BOOST_CHECK(TestSplitHost("::22556", "::22556", 0));
    BOOST_CHECK(TestSplitHost(":22556", "", 22556));
    BOOST_CHECK(TestSplitHost("[]:22556", "", 22556));
    ...
    BOOST_CHECK(TestSplitHost("www.bitcoincore.org:65536",
                              "www.bitcoincore.org:65536", 0, false));
    BOOST_CHECK(TestSplitHost("www.bitcoincore.org:0", "www.bitcoincore.org", 0,
                              false));
...
BOOST_AUTO_TEST_CASE(netbase_lookupnumeric) {
    ...
    BOOST_CHECK(TestParse("127.0.0.1:22556", "127.0.0.1:22556"));
    BOOST_CHECK(TestParse("::ffff:127.0.0.1", "127.0.0.1:65535"));
    BOOST_CHECK(TestParse("::", "[::]:65535"));
    BOOST_CHECK(TestParse("[::]:22556", "[::]:22556"));
    BOOST_CHECK(TestParse("[127.0.0.1]", "127.0.0.1:65535"));
...
BOOST_AUTO_TEST_CASE(onioncat_test) {
    ...
    BOOST_CHECK(addr1.ToStringAddr() == "5wyqrzbvrdsumnok.onion");
...
BOOST_AUTO_TEST_CASE(subnet_test) {
    ...
    const CNetAddr tor_addr{ResolveIP(
    ...
    BOOST_CHECK(!subnet.Match(ResolveIP(
...
BOOST_AUTO_TEST_CASE(netpermissions_test) {
    ...
    BOOST_CHECK(!NetWhitebindPermissions::TryParse(
        "out,forcerelay@1.2.3.4:32", whitebindPermissions, error));
    BOOST_CHECK(error.original.find("whitebind may only be used for incoming "
    ...
    BOOST_CHECK(!NetWhitelistPermissions::TryParse(
        "bloom,forcerelay,noban@1.2.3.4:32", whitelistPermissions,
    ...
    BOOST_CHECK(NetWhitelistPermissions::TryParse(
        "noban@1.2.3.4", whitelistPermissions, connection_direction, error));
    ...
    BOOST_CHECK(NetWhitelistPermissions::TryParse(
        ...
        whitelistPermissions, connection_direction, error));
    BOOST_CHECK_EQUAL(
...
static const std::vector<CAddress> fixture_addresses(
    {CAddress{
         ...
         NODE_NONE, NodeSeconds{0x4966bc61s} /* Fri Jan  9 02:54:25 UTC 2009 */
     },
     CAddress{
