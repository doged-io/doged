...
# USAGE:  ./contrib/tracing/log_utxocache_flush.py path/to/dogecashd
...
def main(dogecashd_path):
    dogecashd_with_usdts = USDT(path=str(dogecashd_path))

    # attaching the trace functions defined in the BPF program
    # to the tracepoints
    dogecashd_with_usdts.enable_probe(probe="flush", fn_name="trace_flush")
    b = BPF(text=program, usdt_contexts=[dogecashd_with_usdts])
...
if __name__ == "__main__":
    if len(sys.argv) < 2:
        print("USAGE: ", sys.argv[0], "path/to/dogecashd")
        exit(1)
