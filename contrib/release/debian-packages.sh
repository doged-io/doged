...
-v, --version <version>   Set the package version. Defaults to the version returned by 'dogecoind --version'.
...
if [ -z "${PACKAGE_VERSION}" ]; then
  PACKAGE_VERSION=$(./src/dogecoind --version | grep -Eo "[0-9]+\.[0-9]+\.[0-9]+")
fi
