...
-v, --version <version>   Set the package version. Defaults to the version returned by 'dogecashd --version'.
...
if [ -z "${PACKAGE_VERSION}" ]; then
  PACKAGE_VERSION=$(./src/dogecashd --version | grep -Eo "[0-9]+\.[0-9]+\.[0-9]+")
fi
